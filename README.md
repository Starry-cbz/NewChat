...
English · [简体中文](./README_zh-CN.md) · [日本語](./README_ja-JP.md) · [Docs](https://newchat.com) · [Discord](https://discord.gg/rpzNSmqaF2) · [Deployment Guide](https://newchat.com/docs/deploy)
...
4. 👀 **Multi-theme Support**: Support for multiple theme switching, including **Light Mode** for light themes and **Dark Mode** for dark themes. 👉 [Custom Color Scheme](https://github.com/Deeptrain-Community/newchat/blob/main/app/src/assets/globals.less)
5. 📚 **Internationalization Support**: Support for internationalization, multi-language switching 🇨🇳 🇺🇸 🇯🇵 🇷🇺 👉 Welcome to contribute translations [Pull Request](https://github.com/Deeptrain-Community/newchat/pulls)
...
9. 📖 **Rich File Parsing**: **Out-of-the-box**, supports file parsing for **all models** (PDF / Docx / Pptx / Excel / image formats parsing), **supports more cloud image storage solutions** (S3 / R2 / MinIO etc.), **supports OCR image recognition** 👉 See project [New Chat Blob Service](https://github.com/Deeptrain-Community/newchat-blob-service) for details (supports Vercel / Docker one-click deployment)
...
> - ...
>
> [👉 Learn More](https://www.newchat.com/docs/contact)
...
## 📦 Deployment
> [!TIP]
> **After successful deployment, the admin account is `root`, with the default password `newchat123456`**
...
> 2. After deployment is complete, please visit your domain name and log in to the backend management using the username `root` and password `newchat123456`. Please follow the prompts to change the password in the newchat backend in a timely manner.
...
7. Visit your configured domain or server `http://[ip]:[port]`, log in to the backend management using username `root` and password `newchat123456`.
...
5. Click the address in **Use Now** on the details interface to enter the CoAI interface. The default username is `root` and the password is `newchat123456` to log in to the backend management.
...
> 
> To use the stable version, use `starrycbz/newchat:stable` instead of `starrycbz/newchat:latest`
...
nohup ./newchat > output.log & # using nohup to run in background
```
...
Thus, **New Chat** was born. We hope to create a project that has both a powerful API distribution system and a rich user interface design, becoming the next-generation open-source AIGC project's one-stop commercial solution.
