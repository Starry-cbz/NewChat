...
[English](./README.md) · 简体中文 · [文档](https://newchat.com) · [Discord](https://discord.gg/rpzNSmqaF2) · [部署文档](https://newchat.com/docs/deploy)
...
4. 👀 **多主题支持**: 支持多种主题切换，包含亮色主题的**明亮模式**和暗色主题的**深色模式**。 👉 [自定义配色](https://github.com/Deeptrain-Community/newchat/blob/main/app/src/assets/globals.less)
5. 📚 **国际化支持**: 支持国际化，支持多语言切换 🇨🇳 🇺🇸 🇯🇵 🇷🇺 👉 欢迎贡献翻译 [Pull Request](https://github.com/Deeptrain-Community/newchat/pulls) 
...
9. 📖 **丰富文件解析**: **开箱即用**, 支持**所有模型**的文件解析 (PDF / Docx / Pptx / Excel / 图片等格式解析), **支持更多云端图片存储方案** (S3 / R2 / MinIO 等), **支持 OCR 图片识别** 👉 详情参见项目 [New Chat Blob Service](https://github.com/Deeptrain-Community/newchat-blob-service) (支持 Vercel / Docker 一键部署)
...
> ### ✨ New Chat 商业版
> ![商业版预览](./screenshot/newchat-pro.png)
>
...
> - ...
>
> [👉 了解更多](https://www.newchat.com/docs/contact)
...
## 📦 部署方式
> [!TIP]
> **部署成功后, 管理员账号为 `root`, 密码默认为 `newchat123456`**
...
> 2. 部署完成后, 请访问你的域名, 并使用用户名 `root` 密码 `newchat123456` 登录后台管理，请按照提示在 newchat 后台及时修改密码。
...
7. 访问您配置的域名或服务器 `http://[ip]:[port]`，使用用户名 `root` 和密码 `newchat123456` 登录后台管理。
...
5. 点击详情界面**立即使用**中的链接，可进入CoAI社区版界面。默认用户名为`root`，密码`为newchat123456` 登录后台管理。
...
> 
> 如需使用 stable 版本, 请使用 `starrycbz/newchat:stable` 替代 `starrycbz/newchat:latest`  
...
4. **我的机器为 ARM 架构, 该项目支持 ARM 架构吗？**
   - 支持。New Chat 项目使用 BuildX 构建多架构镜像, 你可以直接使用 docker-compose 或 docker 运行, 无需额外配置。
   - 如果你使用编译安装, 直接在 ARM 机器上编译即可, 无需欸外配置。如果你使用 x86 机器编译, 请使用 `GOARCH=arm64 go build -o newchat` 进行交叉编译并上传至 ARM 机器上运行。
5. **如何修改 Root 默认密码？**
...
7. **如何配置支付方式？**
   - New Chat 开源版支持发卡模式, 设置系统设置中的购买链接为你的发卡地址即可。卡密可通过用户管理中兑换码管理中批量生成。
8. **礼品码和兑换码有什么区别？**
...
9. **该项目支持 Vercel 部署吗？**
   - New Chat 本身并不支持 Vercel 部署, 但是你可以使用前后端分离模式,  Vercel 部署前端部分, 后端部分使用 Docker 部署或编译部署。
10. **前后端分离部署模式是什么？**
    - 正常情况下, 前后端在同一服务内, 后端地址为 `/api`。前后端分离部署指前端和后端分别部署在不同的服务上, 前端服务为静态文件服务, 后端服务为 API 服务。
      - 举个例子, 前端使用 Nginx (或 Vercel 等) 部署, 部署的域名为 `https://www.newchat.net`。
      - 后端使用 Docker 部署, 部署的域名为 `https://api.newchat.net`。
    - 此种部署方式需自行打包前端, 配置环境变量 `VITE_BACKEND_ENDPOINT` 为你的后端地址, 如 `https://api.newchat.net`。
...
14. **如何开始域名严格跨域检测?**
    ...
    - 如果需要开启严格跨域检测，可以在后端环境变量中 并配置 `ALLOW_ORIGINS`, 如 `ALLOW_ORIGINS=newchat.net,newchat.app` （不需要加协议前缀, www 解析无需手动添加, 后端将自动识别并允许跨域）, 这样就会支持严格跨域检测 (如 *http://www.newchat.app*, *https://newchat.net* 等将会被允许, 其他域名将会被拒绝)。
...
于是，**New Chat** 应运而生，我们希望能够做出一个既有强大的 API 分发系统，又有丰富的用户界面设计的项目，成为下一代开源 AIGC 项目的商业一站式解决方案。
