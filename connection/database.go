...
func getConn() *sql.DB {
	if viper.GetString("mysql.host") == "" {
		...
		globals.Warn("[connection] mysql host is not set, using sqlite (~/db/newchat.db)")
		db, err := sql.Open("sqlite3", utils.FileSafe("./db/newchat.db"))
		if err != nil {
			panic(err)
		}
...
func InitRootUser(db *sql.DB) {
	...
	if count == 0 {
		globals.Debug("[service] no user found, creating root user (username: root, password: newchat123456, email: root@example.com)")
		_, err := globals.ExecDb(db, `
			INSERT INTO auth (username, password, email, is_admin, bind_id, token)
			VALUES (?, ?, ?, ?, ?, ?)
		`, "root", utils.Sha2Encrypt("newchat123456"), "root@example.com", true, 0, "root")
		if err != nil {
